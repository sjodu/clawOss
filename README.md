...
<p align="center">
<code>[AUTONOMOUS]</code> <code>[V11]</code> <code>[FLEXIBLE LLM]</code> <code>[BUDGET CONTROL]</code>
</p>
...
# 1. Configure your LLM and budget
cp .env.example .env
# Edit .env with your settings:
#   - LLM_PROVIDER (openai, anthropic, deepseek, etc.)
#   - LLM_MODEL (gpt-4o-mini, claude-3-5-sonnet, etc.)
#   - LLM_API_KEY (your API key)
#   - BUDGET_MAX_USD (e.g., 10.0)
...
ClawOSS V11 supports **any OpenAI-compatible LLM**:
...
V11 includes automatic budget control to prevent cost overruns:

```bash
# Check current budget status
bash scripts/budget-status.sh

# Reset budget counter (after reviewing costs)
bash scripts/budget-reset.sh
```

The agent automatically stops when the budget limit is reached.
