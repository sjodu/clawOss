...
**0d. Budget check** (V11: prevent quota overrun):
```bash
BUDGET_MAX=$(cat workspace/memory/budget-max.txt 2>/dev/null || echo "100.0")
BUDGET_SPENT=$(cat workspace/memory/budget-spent.txt 2>/dev/null || echo "0.0")
```
Compare spent vs max. If `BUDGET_SPENT >= BUDGET_MAX`:
- **STOP ALL WORK IMMEDIATELY** — do NOT proceed to any other steps
- Log: "Budget exhausted: $BUDGET_SPENT / $BUDGET_MAX. Pausing until budget reset."
- Exit heartbeat loop — the agent will resume when budget is reset via `bash scripts/budget-reset.sh`
If budget OK, log current status and continue: "Budget OK: $BUDGET_SPENT / $BUDGET_MAX"
