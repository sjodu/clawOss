...
// Dynamic LLM pricing from environment variables (V11: flexible model support)
  ...
  try {
    ...
    if (event.type === "agent_end" || event.action === "agent_end") {
      ...
      if (accumulatedInputTokens > 0 || accumulatedOutputTokens > 0) {
        ...
        // Accumulate cost to budget-spent.txt
        try {
          const fs = await import("fs");
          const budgetSpentPath = `${process.cwd()}/workspace/memory/budget-spent.txt`;
          let totalSpent = 0.0;
          try {
            const content = fs.readFileSync(budgetSpentPath, "utf-8");
            totalSpent = parseFloat(content.trim()) || 0.0;
          } catch {
            // File doesn't exist yet, start from 0
          }
          totalSpent += costUsd;
          fs.writeFileSync(budgetSpentPath, totalSpent.toFixed(6));

          // Log budget status
          console.log(`[dashboard-reporter] Budget: $${totalSpent.toFixed(2)} / $${BUDGET_MAX_USD.toFixed(2)} (this run: $${costUsd.toFixed(4)})`);
        } catch (err) {
          console.error("[dashboard-reporter] Failed to update budget:", err);
        }
